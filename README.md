...
[HTTP](https://www.mnot.net/blog/2014/06/07/rfc2616_is_dead) interface to a [filesystem](http://www.multicians.org/fjcc4.html). one [use](http://suckless.org/philosophy) is a webserver for [mail](conf/mail/) and [news](conf/news/) ([message/rfc2822](http://www.faqs.org/rfcs/rfc2822.html), [RSS](http://web.resource.org/rss/1.0/spec), [Atom](https://tools.ietf.org/html/rfc4287)). [on-line search](https://en.wikipedia.org/wiki/Online_search) is available via [Groonga](http://groonga.org/) and [grep](http://www.gnu.org/software/grep/manual/grep.html). no server-side [crawlers](https://en.wikipedia.org/wiki/Web_crawler): you must [GET](ruby/read.rb.html) to trigger indexing of stored [content](https://en.wikipedia.org/wiki/Content_(media))
...
#### LDP
our server is mostly [LDP-compatible](http://www.w3.org/TR/ldp/). we use SOLID/LDP apps by delegating pure-RDF features like SPARQL-update PATCHes to the appropriate libraries, though we're not fully test-suite compliant as we've done things like only support one Container type. RDF now has a LDP library under heavy-development with its own HTTP handlers and daemon. our daemon has a cascading-container routing-engine for custom handlers (thread/discussion-reconstruction for email/forums, HTTP-header overrides for serving Atom at /feed, enabling pagination at /news for an "infinite feed"), and customization-hooks for plain-HTML view-templates and pagination/chunking while their daemon currently depends on all of Sinatra and doesnt do this other stuff, and also has code doing stuff we dont want and might have to monkeypatch, much like Rails and Sinatra did which is why we wrote our own daemon). but especially as the LDP spec has Non-RDF awareness written into it we are therefore a prime-candidate to figure out how to merge our daemon with theirs without losing any of the performance and functionality. it's on the **TODO** list and we love rewriting so you may be in luck

#### User

one reason we serve RDF is so you can [bring your own](https://github.com/solid/solid-apps) interface easily. lynx/links/w3m-compatibility (non-JS hypertext-browsers) is important to us so we provide text/html on request
