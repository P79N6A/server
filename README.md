...
#### LDP
our server is mostly [LDP-compatible](http://www.w3.org/TR/ldp/). we use SOLID/LDP apps by delegating pure-RDF features like SPARQL-update PATCHes to the appropriate libraries, though we're not fully test-suite compliant as we've done things like only support one Container type. RDF now has a LDP library under heavy-development with its own HTTP handlers and daemon. our daemon has some custom handlers (thread/discussion-reconstruction for email/forums, HTTP-header overrides for serving Atom at /feed, flipping-on pagination at /news for an "infinite feed"), a variety of HTML view-templates and custom-paginators. we are a prime-candidate to figure out how to merge our daemon with theirs without losing any of the performance and functionality. it's on the **TODO** list
