...
class R
    ...
    [{class: :container, style: "background-color: #{R.cs}",
      c: [{_: :a, class: :uri, c: r[Label] || (re.path=='/' ? re.host : re.abbr), href: re.uri}, ' ',
          r[LDP+'contains'].do{|c|
            ...
            [(H.once(e,:sort,{_: :a, class: :sort, c: sortLabel, href: (re.path||'')+'?sort='+s_, title: s_}) if c.size > 2 && (re.host==e['HTTP_HOST'] || !re.host)),
             ('<br>' if c.size > 1),
             c.sort_by{|i|
  ...
  ViewA[LDP+'Resource'] = -> u,e {
    [u[Prev].do{|p|{_: :a, rel: :prev, href: p.uri, c: ['&larr; ', p.R.offset], title: '↩ previous page'}},
