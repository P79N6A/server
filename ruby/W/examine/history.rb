...
class E
  ...
  def triplrMozHist
    ...
    query = "select p.url, r.url, h.visit_date, hr.visit_date from moz_places as p,moz_places as r, moz_historyvisits as h, moz_historyvisits as hr where p.id = h.place_id and r.id = hr.place_id and hr.id = h.from_visit #{q} #{t} order by h.visit_date desc limit #{c}"

    puts "query #{query}"

    `sqlite3 -separator "\t" #{sh} "#{query}"`.
      lines.to_a.map{|l| # each line
      l = l.split /\t/   # tab-separatio
      puts "line "+l.join(" ")
