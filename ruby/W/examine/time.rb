...
class E
  ...
  fn 'filter/tg',->e,m{
    m.values.group_by{|r|[*r[e['group']]][0]}.map{|g,v|
      p=nil
      v.sort_by{|r|[*r[e['xaxis']||Date]][0].to_time}.map{|r|
        r['/parent']=[p]
        p=r}}}
  ...
  fn 'view/tg/base',->d,e,c{ e[:graph]||=d
    ...
    o=d.map{|_,r|r[e.q['yaxis']]}.flatten.compact.map &:to_f # other axis
    ...
    d.map{|_,r| r[a] &&
      (t = e.q['group'].do{|g|[*r[g.expand]][0]}||[*r[Title]][0].sub(/^[rR][eE][^A-Za-z]./,'') # group id
       ...
       r['top']=e.q['yaxis'].do{|a|[*r[a]][0].do{|v|(omax - v.to_f)*oscale}||0}|| rand(100) # y position
       # group options
       r['group'] = (g[t] ||=
                 {c: '#%06x' % rand(16777216),
                   a: r.url,
  ...
  fn 'view/tg/item',->r,x{ r[x.q['xaxis']||Date] &&
    (t = r['top'].to_s+'%'
     l = r['left'].to_s+'%'
