...
class R
  ...
  def R.call e
    ...
    rawpath = Pathname.new CGI.unescape e['REQUEST_PATH'].force_encoding 'UTF-8'
    path = rawpath.expand_path.to_s                                         # interpret path
    path = path + ((path[-1] != '/' && rawpath.to_s[-1] == '/') ? '/' : '') # preserve trailing slash
